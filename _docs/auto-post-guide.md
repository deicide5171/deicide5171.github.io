...
- **하루 목표: 14편** (필러 7개 × 2라운드, 필러당 2편). 각 편은 두 가지 형식으로 생성한다.
  ...
  2. 네이버 블로그용 HTML → `_naver/<slug>.html` (날짜 없음)
...
경로: **`_naver/<slug>.html` — 날짜 접두사를 붙이지 않는다** (예: `_naver/cors-error-basics.html`).
같은 주제를 네이버 독자용(~습니다체)으로 다시 쓴 문서. 마크다운 문법 금지.
(네이버는 사용자가 날짜와 무관하게 원하는 글을 골라 올리므로 날짜가 오히려 방해가 된다.
`_posts/`의 마크다운은 Jekyll 요구사항이라 반드시 날짜를 유지해야 하지만, `_naver/`는 날짜 없이 slug만 쓴다.
slug는 저장소 전체에서 고유하므로 충돌하지 않는다.)

**폴더 규칙**:
- `_naver/` 루트 = **네이버 미게시 대기함** — 새 파일은 반드시 여기에 `<slug>.html` 이름으로 생성
